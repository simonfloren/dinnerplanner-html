...
class DinnerModel {
  ...
  getDishTypes() {
	...
	getBulkDishInfo(ids) {
		const url = baseURL + "/recipes/informationBulk";
		return fetch(url, {
			method: "GET",
		})
	}
  ...
  getRandomDish() {
		...
		return fetch(url,{	
			...
			.then(data => {
				console.log(data);
				let dishIds = [];
				data.recipes.forEach(currentDish => {
					const { id } = currentDish;
					dishIds.push(id);
				});

				// fetch better information about dishes
				getBulkDishInfo(dishIds);
			});
	}
